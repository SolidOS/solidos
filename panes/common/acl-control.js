...
UI.widgets.preventBrowserDropEvents = function (document) {
...
UI.widgets.ACLControlBox = function (subject, dom, noun, callback) {
  var kb = UI.store
  ...
  var ACLControlEditable = function (box, doc, aclDoc, kb, options) {
    ...
    options = options || {}
    var ac = UI.widgets.readACL(doc, aclDoc, kb, options.doingDefaults) // Note kb might not be normal one
    var byCombo
    box[defaultOrMain] = byCombo = UI.widgets.ACLbyCombination(ac)
    ...
    var agentTriage = function (uri) {
      var ns = UI.ns, kb = UI.store, obj = $rdf.sym(uri)
      var obj = $rdf.sym(uri)
    ...
    box.saveBack = function (callback) {
      var kb2 = $rdf.graph()
      if (!box.isContainer) {
        UI.widgets.makeACLGraphbyCombo(kb2, doc, box.mainByCombo, aclDoc, true)
      } else if (box.defaultsDiffer) { // Pair of controls
        UI.widgets.makeACLGraphbyCombo(kb2, doc, box.mainByCombo, aclDoc, true)
        UI.widgets.makeACLGraphbyCombo(kb2, doc, box.defByCombo, aclDoc, false, true)
      } else { // Linked controls
        UI.widgets.makeACLGraphbyCombo(kb2, doc, box.mainByCombo, aclDoc, true, true)
      }
      ...
      if (defa) {
        ...
        if (!defaults.length) {
          ...
          statusBlock.innerHTML = 'The sharing for this ' + noun + " is the default for folder <a href='" + defaultHolder.uri + "'>" +
            UI.utils.label(defaultHolder) + '</a>.'
          var kb2 = UI.widgets.adoptACLDefault(doc, targetACLDoc, defaultHolder, defaultACLDoc)
        ...
        var addDefaultButton = function (prospectiveDefaultHolder) {
          ...
          useDefault.addEventListener('click', function (event) {
            UI.store.fetcher.webOperation('DELETE', targetACLDoc)
              .then(function (xhr) {
                UI.store.fetcher.requested[targetACLDoc.uri] = 404
                UI.store.fetcher.nonexistant[xhr.resource.uri] = true
                UI.store.fetcher.unload(targetACLDoc)
        ...
        if (targetDocDir) {
          var prospectiveDefaultHolder
          UI.widgets.getACLorDefault($rdf.sym(targetDocDir), function (ok2, p22, targetDoc2, targetACLDoc2, defaultHolder2, defaultACLDoc2) {
            if (ok2) {
        ...
        if (box.isContainer) {
          var ac = UI.widgets.readACL(targetDoc, targetACLDoc, kb)
          var acd = UI.widgets.readACL(targetDoc, targetACLDoc, kb, true)
          box.defaultsDiffer = !UI.widgets.sameACL(ac, acd)
