...
UI.widgets.adoptACLDefault = function (doc, aclDoc, defaultResource, defaultACLdoc) {
  var kb = UI.store
  var ACL = UI.ns.acl
  var ns = UI.ns
...
//
UI.widgets.readACL = function (x, aclDoc, kb, getDefaults) {
  var kb = kb || UI.store
  var ns = UI.ns
...
// Compare two ACLs
UI.widgets.sameACL = function (a, b) {
...
// Union N ACLs
UI.widgets.ACLunion = function (list) {
  var b = list[0], a, ag
...
UI.widgets.loadUnionACL = function (subjectList, callback) {
  var aclList = []
...
//
UI.widgets.ACLbyCombination = function (ac) {
...
//
UI.widgets.makeACLGraph = function (kb, x, ac, aclDoc) {
  var byCombo = UI.widgets.ACLbyCombination(ac)
  return UI.widgets.makeACLGraphbyCombo(kb, x, byCombo, aclDoc)
}

//    Write ACL graph to store from combo
//
UI.widgets.makeACLGraphbyCombo = function (kb, x, byCombo, aclDoc, main, defa) {
  var ACL = UI.ns.acl
...
//
UI.widgets.ACLToString = function (ac) {
  return UI.widgets.comboToString(
    UI.widgets.ACLbyCombination(ac))
}
UI.widgets.comboToString = function (byCombo) {
  str = ''
  for (combo in byCombo) {
    var modeURIs = combo.split('\n')
...
//
UI.widgets.makeACLString = function (x, ac, aclDoc) {
  var kb = $rdf.graph()
...
//
UI.widgets.putACLObject = function (kb, x, ac, aclDoc, callback) {
  var byCombo = UI.widgets.ACLbyCombination(ac)
...
//
UI.widgets.putACLbyCombo = function (kb, x, byCombo, aclDoc, callback) {
  var kb2 = $rdf.graph()
...
UI.widgets.fixIndividualCardACL = function (person, log, callback) {
...
UI.widgets.fixIndividualACL = function (item, subjects, log, callback) {
  log = log || console.log
  var doc = item.doc()
...
UI.widgets.setACL = function (docURI, aclText, callback) {
...
UI.widgets.getACLorDefault = function (doc, callback) {
...
//
UI.widgets.getACL = function (doc, callback) {
